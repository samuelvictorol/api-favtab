...
async function verificaUsuario(usuarioReq) {
    const usuario = await UsuarioModel.findOne(usuarioReq.login);
    if (!usuario) {
        return 'Usuário não encontrado'
    } else{
        if (!usuario.senha == usuarioReq.senha) {
...
const repertorioController = {
    novoRepertorio: async (req, res) => {
        const { nome, descricao, genero, private, musicas } = req.body;
        console.log(req.body);
        try {
            const usuarioReq = { {login}, senha: password };
            const checkUser = await verificaUsuario(usuarioReq);
            if(checkUser !== true){
