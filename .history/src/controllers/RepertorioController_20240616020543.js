...
const repertorioController = {
    novoRepertorio: async (req, res) => {
        ...
        try {
            if(!await UsuarioManager.verificaUsuario({login: login, senha: password})){
    ...
    getMeusRepertorios: async (req, res) => {
        ...
        if (!await UsuarioManager.verificaUsuario({login: login, senha: senha})) {
            return res.status(400).json({
                message: 'Credenciais inválidas existe',
            });
        }
        ...
        try {
            let repertorios = []
            if (!senha || senha.trim() === '') {
                console.log('Senha é obrigatória para acessar repertórios privados');
                repertorios = await RepertorioModel.find({criadoPor: login})
                ...
                if (!usuario) {
                    ...
                    return res.status(401).json({
                        message: 'SENHA DEVE SER CRIPITOGRAFADA [bcrypt]',
                    });
                }
            }
    ...
    createMusica: async (req, res) => {
        const { usuario, senha, musica, repertorioId } = req.body;
